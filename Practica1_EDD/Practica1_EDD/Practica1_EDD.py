...
class Funciones:
       ...
       def MenuUsuairo(self,nombre):    #---------------------------- Menu usuario -------------------------------------------------------------
          ...
          while (salida==True):
              ...
              if entrada=="6":
                ...
                print("*-------------------------------* Menu de Usuario -> Leer archivo*------------------------------*")
       ...
       def MenuOperarMatriz(self,nombre):
           salir=True
           tempo=usuarios.DameUsuario(nombre)
           while (salir==True):
              ...
              if entrada=="5":
                  ...
                  self.IngresarValorEnMatriz(posx,posy,valor,nombre)
                  #print("Dato ingresado")
              elif entrada=="2":
                  """operar transpuesta"""
                  if tempo.matriztrans==None:
                      tempo.matriztrans=MatrizOrtogonal()
                      self.OperarTranspuesta(nombre)
                      #print("La matriz no ha sido creada.")
                  #self.OperarTranspuesta()
              elif entrada=="3":
                  """mostrar matriz original""" #------------------------------------------- Mostrar matriz original ------------------                
                  if tempo.matriz!=None:
                    print("-------------------------------->  Matriz Original")
                    tempo.matriz.MostrarMatriz()
                  else:
                      print("La matriz no ha sido creada.")
              elif entrada=="4":
                  """mostrar transpuesta"""
                  if tempo.matriztrans!=None:
                      ...
                      tempo.matriztrans.MostrarMatriz()
                  else:
                      print("La matriz no ha sido creada.")
                  #MatrizTranspuesta.MostrarMatriz()
              else:
                  print("Ingrese una opcion valida!")

       def OperarTranspuesta(self,nombre):
           """"""
           temporal=usuarios.DameUsuario(nombre)
           temporal.matriztrans.CrearMatriz(temporal.matriz.Y,temporal.matriz.X)
           #alto=Matriz.X
           #ancho=Matriz.Y
           for j in range(temporal.matriztrans.Y):
               for i in range(temporal.matriztrans.X):
                   temp=temporal.matriz.DameValor(j,i)
                   temporal.matriztrans.MeterValor(i+1,j+1,temp)
           print("Matriz Transpuesta creada exitosamente!")
           #MatrizTranspuesta.MostrarMatriz()

       def IngresarValorEnMatriz(self,posx,posy,valor,nombre): #-------------------- Meter datos a la matriz ---------------------------
           tempo=usuarios.DameUsuario(nombre)
           tempo.matriz.MeterValor(int(posx),int(posy),valor)
           #Matriz1=Matriz
       ...
       def LeerArchivo(self,ruta,nombreuser):  #--------------------------------- Leer archivo  ------------------------
           ...
           if temporal.matriz==None:
              temporal.matriz=MatrizOrtogonal()
              temporal.matriz.CrearMatriz(int(xx),int(yy))
...
class MatrizOrtogonal: #--------------------------------------------- Matriz Ortogonal ------------------------------------------------
    ...
    def MeterValor(self,posx,posy,valor):
        if ((posx-1)<self.X  and (posy-1)<self.Y):
           cabeza=self.ultima.DamePrimero()
           while(cabeza.DameArriba()!=None):
               cabeza=cabeza.DameArriba()

           for i in range(posy-1):
               cabeza=cabeza.DameAbajo()
    ...
    def CrearMatriz(self,ancho,alto):
        ...
        if (self.X>0 and self.Y>0):
            self.ultima=ListaMatriz(self.X)
            for i in range(self.Y):
                aux=ListaMatriz(self.X)
                recorre=aux.DamePrimero()
                recorreUlitima=self.ultima.DamePrimero()
                ...
                self.ultima=aux
                self.ultima.MostrarListaMatriz()
        ...
        if (self.X>0 and self.Y>0):
            ...
            for i in range(self.Y):
                ...
                for j in range(self.X):
                    #print(str(self.DameValor(j,i)))
                    text+=" "+ str(self.DameValor(j,i))
                text+=" |\n"
            #text+="|"
